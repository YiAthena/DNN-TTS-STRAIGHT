...
#$prjdir="/home/zhaoyi/mywork/tts/HTStry/Demo/CMU_STRAIGHT";
#require( $ARGV[0] );
$prjdir=$ARGV[0];
$datadir="$prjdir/data";
$PHONE="$datadir/labels/full";
$DNNDATA="$datadir/DNNDATA3";
$List="$DNNDATA/list";
@filelist=glob("$PHONE/*.lab");
print "$#filelist +1 files found \n";
&SetSeqSymbol();
&SetCarte();
$para_N=53;
$fileExist=-e $DNNDATA;
...
1,#    e3 position of the current word in the current phrase (forward)
1,#    e4 position of the current word in the current phrase (backward)
